...
def get_news_data(notification_mode = "email"):
    try:
        all_articles = []
        ...
        # -----------------------------
# FIX TIMEZONE ISSUE (EXCEL SAFE)
# -----------------------------
        news_df["published_at"] = pd.to_datetime(news_df["published_at"], utc=True).dt.tz_localize(None)
        news_df["collected_at"] = pd.to_datetime(news_df["collected_at"]).dt.tz_localize(None)
        ...
        spike_df = sentiment_news_spike.new_sentiment_spike(news_df)
        if spike_df.empty:
            message = (
            f"News data extracted successfully.\n"
            f"Total articles collected: {len(news_df)}"
            )
        else:
            message = (
                "News sentiment spikes detected!\n"
                "Please check the attached spike report for details."
            )
        if notification_mode == "email":
            send_mail(
                subject="News Sentiment Monitoring Report",
                text = message,
                df=news_df if not news_df.empty else None
                )
        else:
        ...
        error_msg = f"Failed to Extract News Data and the reason is: {e}"
        print(error_msg)
        if notification_mode == "email":
            send_mail(
...
# get_news_data(notification_mode = "email")
# get_news_data(notification_mode = "slack")
if __name__ == "__main__":
    get_news_data(notification_mode="email")  # or "slack"