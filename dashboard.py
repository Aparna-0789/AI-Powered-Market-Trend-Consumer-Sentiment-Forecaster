...
import external_api.Rapid_api as rapid
...
def run_scheduler():
    ...
    #schedule.every().day.at("16:47").do(news.get_news_data)
    schedule.every(10).sunday.at("02:00").do(news.get_news_data)
    schedule.every(10).sunday.at("03:00").do(reddit.get_reddit_data)
    schedule.every(10).sunday.at("04:00").do(rapid.get_rapid_data)

    
    # schedule.every(10).monday.at("02:00").do(notification.testing_function)
    #schedule.every().day.at("22:00").do(rapid_api.get_rapid_data)
    #schedule.every().day.at("22:30").do(reddit_api.get_reddit_data)
    while True:
...
"""
            ...
            try:
                ...
                st.warning("Gemini failed - switching to Groq")
                try:
        ...
        if response_text is not None:
           st.success(f"Insight Generated using {model_used}")
